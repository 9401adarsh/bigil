...
def imageRetriever(image_path, text_path=None):
    ...
    if(text_path == None):
        ...
        for tf in tf_list:
            ...
            if cmd[0] == "GB":
                imageObject = imageObject.filter(ImageFilter.GaussianBlur(radius=int(cmd[1])))
                #imageObject.show()
            elif cmd[0] == "B":
                ...
                t_img = imageObject.save(temp_path, optimize=True, quality=int(cmd[1]))
...
i = imageRetriever("test_imgs/lena.jpg","transformLogExample.txt")
i.show()